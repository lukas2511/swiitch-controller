...
int poll_controller(void) {
  if(i2c_read(0x52, controller_state, 6) == 6) {
    switch_controller.data.RX = (_GETBITS(controller_state[0], 6, 2) << 3 | _GETBITS(controller_state[1], 6, 2) << 1 | _GETBITS(controller_state[2], 7, 1)) << 3;
    switch_controller.data.RY = _GETBITS(controller_state[2], 0, 5) << 3;

    switch_controller.data.LX = _GETBITS(controller_state[0], 0, 6) << 2;
    switch_controller.data.LY = _GETBITS(controller_state[1], 0, 6) << 2;

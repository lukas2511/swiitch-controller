...
void sys_tick_handler(void)
{
  ...
  for(int i=0; i<sizeof(buttons.bytes); i++) {
    buttons.bytes[i] = 0;
  }
  ...
  static int y = 0;
/*  if(y==0) {
    if(x<=7) { _SETBIT(buttons.bytes[0], x); } else { _SETBIT(buttons.bytes[1], x-8); }
    if(dir == 1 && x == 13) dir=-1;
    if(dir == -1 && x == 0) dir=1;
    x = x+dir;
  }*/
  y+=1;
  buttons.data.dpad_up = 0;
  if(y>100) {
    buttons.data.dpad_up = 1;
  }
  if(y>200) {
    y=0;
  }

  buttons.data.LX = 127 + 64;
  buttons.data.LY = 127;
  buttons.data.RX = 127;
  buttons.data.RY = 127;

  anal++; if(anal == 0xff) anal=0;

	usbd_ep_write_packet(usbd_dev, 0x81, buttons.bytes, sizeof(buttons.bytes));
}
